...
shape: federated short-lived credentials whose trust config is code.
...
same shape either way. Ecosystem is healthy (active repos, hardened helm
...
1. The `src/trust/` typed form — one shape across the three issuer
