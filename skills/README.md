...
| `i9-federation-trust` | IAM lesson 9, Federation trust. Worktree at `checkpoint/i8`, federates `site-publisher` through the GitHub Actions anchor with the `github-pages` environment as the exact subject, refuses a wildcard at validate, writes `trust.rego` and the subject fixtures, brings in the audience fixtures and the rotation machinery, pages on a hand-edited trust policy and a widened anchor, runs `rotation` clean and then against a hand-made key under a zero window, forges a token to show the emulator failing open, and closes the workflow-level gap in `check workflow`. Reuses `skills/start/check.sh`. | first draft |
