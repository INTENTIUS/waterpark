...
*open* need design before the lesson that depends on them is written.
...
2. **The CI runner.** A compromised runner in an apply job is full
   ...
   self-hosted posture *open*. (A Fountain self-hosted runner is a
   different thing and is trusted mode, decision 29; it never hosts an
   apply job.)
...
5. **The concierge agent.** An agent authors PRs from plain-language
   ...
   verbs are served by a process outside the boundary — Rounds' server
   is the reference implementation (decision 30): it holds the plan-tier
   credential behind a network-bound trust anchor, itself a principal in
   the repo, and it is the only thing that writes. The sandbox receives
   only a conversation-scoped token, and its egress is a default-deny
   allowlist naming the verb service and the code host (on a hosted
   sandbox provider only, decision 29). Request text is
...
read-tier action list. Each settles in the lesson that needs it: the
boundary policy in I5, routing in I7, signal authentication in I10,
provenance in I8.
