...
| `f4-credentials-and-vaults` | Fountain lesson 4, Credentials and vaults. Applies an Environment and two Vaults overriding one key, then two Agents allowed one vault each by id, starts a conversation on each, reads the merged secret off the runner's disk with `just runner-sh`, gets a disallowed vault refused with a 422, hears the agent decline to print the value, empties an allowlist and sees a new conversation fall back to the environment's value while the parked one keeps the vault's. Four model turns. Reuses `skills/start/check.sh`. | first draft |
