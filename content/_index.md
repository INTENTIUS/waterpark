...
Two courses. The first teaches Fountain and the propose loop. The second applies them to org IAM at a fictional company, pepperoni: an agent holding no cloud credentials builds a working AWS access repo with chant, then works as its concierge and its watcher. Every lesson is one theme, one outcome, one check, and runs alone (Floci, free) or live with a group (real AWS, a facilitator, checkpoints).
