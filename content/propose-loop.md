...
| **read** | A deterministic read of the target. An audit yields findings sorted by fix confidence (deterministic, judgement, report-only). A request yields the current state, re-read before any apply. | An audit over ten catalogs. Silent catalogs are shown greyed. | The same, on a schedule. | `dns-state`, incremental per zone. |
...
|---|---|---|
| target | The access repo and the live estate it owns, plus one satellite repo. | I1, I8 |
...
| verify | The guardrails and a `terraform plan` against Floci on every PR with no credential. `check-no-new-access` where an untrusted author cannot trigger it. The digest re-checked at apply. | I4, I6, I14 |
