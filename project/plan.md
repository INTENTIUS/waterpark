...
| F3 | What the sandbox can reach | `networking_type: limited` is a default-deny allowlist, and a backend that cannot hold one is refused rather than run open | A limited environment refuses to provision on the class stack's runner, naming `backend_lacks_network_policy` and the provider, while the same agent on an unrestricted environment provisions |
