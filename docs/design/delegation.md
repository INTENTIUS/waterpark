# Design: delegated role creation (decision 20)
...
`flume/access` — the ticket queue the product exists to remove, wearing
...
Same shape as break-glass's layering: the convenient layer gives fast
...
| Central (`flume/access`) | Satellite |
|---|---|
...
```ts
// flume-payments/src/roles/queue-consumer.ts — the whole file
