...
| `i13-the-watcher` | IAM lesson 13, The watcher. Reads the estate's three projections from the account rather than from a file, leaves a break-glass grant past its expiry on the base branch and sweeps it into a pull request whose diff removes the block, puts that beside a reconcile pull request whose diff is empty until the student can say which way each pushes, checks that the `pr` job's cap counts both markers against the one number in `access/baseline`, and asks `GET /api/search` which requests the concierge handled. Most of it needs no model turn. Reuses `skills/start/check.sh`. | student run |
