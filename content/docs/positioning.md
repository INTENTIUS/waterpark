...
## Strategic role, revised (decision 26)

The pattern is taught, not sold: the IAM scenario is the deep track of
a course on running agents against infrastructure on Fountain, and the
reason IAM is the deep one is the same reason it would have been the
wedge — small estates, no data-plane risk, high organizational
visibility, and a room that recognizes the pain. The ideal-customer and
market sections above are the audience description for the scenario,
and the place to restart if the kit is ever productized. chant stays
the realization where the pattern is cheapest; the Terraform backend
is an appendix lesson.
